...
const EARLY_NICK_SKIP = new Set([
    'http', 'https', 'training', 'battle', 'Level', 'mouse', 'Enabled', 'Type',
    'account', 'Database', 'switch', 'Teams', 'damage', 'arena', 'Category',
    'ruby', 'apple', 'minor', 'major', 'medium', 'none', 'draw', 'Scale', 'Rect'
]);

function scanPayloadNicknames(payload, found, skip) {
    for (let i = 0; i < payload.length - 4; i += 1) {
        if (payload.readUInt8(i) !== 0x1a) continue;
        const lenPart = readVarint(payload, i + 1);
        if (!lenPart || lenPart.value < 2 || lenPart.value > 24) continue;
        const start = lenPart.next;
        const end = start + lenPart.value;
        if (end > payload.length) continue;
        const nick = payload.subarray(start, end).toString('utf8');
        if (!/^[A-Za-z0-9_\-]{2,24}$/.test(nick)) continue;
        if ([...skip].some((w) => nick.toLowerCase().includes(w.toLowerCase()))) continue;
        if (!found.has(nick)) found.set(nick, { nickname: nick, team: 0, accountId: 0 });
    }
}

function parseEntityMethodPlayers(payload) {
    const players = parseEntityMethodPlayersLegacy(payload);
    if (players.length) return players;
    const found = new Map();
    scanPayloadNicknames(payload, found, new Set());
    return [...found.values()];
}

function parseEntityMethodPlayersLegacy(payload) {
    if (payload.length < 12) return [];
    const subType = payload.readUInt32LE(4);
    if (subType !== 47) return [];
    const players = [];
    let i = 0;
    while (i < payload.length - 3) {
        if (payload.readUInt8(i) !== 0x1a) {
            i += 1;
            continue;
        }
        const lenPart = readVarint(payload, i + 1);
        if (!lenPart || lenPart.value <= 0 || lenPart.value > 64) {
            i += 1;
            continue;
        }
        const start = lenPart.next;
        ...
        const nick = extractProtoString(chunk, 3);
        ...
        const team = extractProtoVarint(chunk, 4);
        const accountId = extractProtoVarint(chunk, 7);
        if (!players.some((p) => p.nickname === nick)) {
            players.push({
                nickname: nick,
                team: team || 0,
...
function extractProtoString(buf, fieldNumber) {
    const tag = (fieldNumber << 3) | 2;
    for (let i = 0; i < buf.length - 2; i += 1) {
        if (buf.readUInt8(i) !== tag) continue;
        const lenPart = readVarint(buf, i + 1);
        if (!lenPart) continue;
        const start = lenPart.next;
        const end = start + lenPart.value;
...
function extractProtoVarint(buf, fieldNumber) {
    const tag = (fieldNumber << 3) | 0;
    for (let i = 0; i < buf.length - 2; i += 1) {
        if (buf.readUInt8(i) !== tag) continue;
        const valPart = readVarint(buf, i + 1);
        if (!valPart) continue;
        return valPart.value;
    }
    return null;
}
