...
app.post('/api/donation-goal/manual-donation', (req, res) => {
    ...
    db.get('SELECT * FROM donation_goals WHERE id = 1', (err, goal) => {
        ...
        if (!goal) {
            console.log('❌ Цель сбора не найдена, создаем новую');
            // Создаем новую цель если не существует
            db.run(`INSERT INTO donation_goals (id) VALUES (1)`, (err) => {
                if (err) {
                    console.error('❌ Ошибка создания цели:', err);
...
function processManualDonation(goal, amount, res) {
    ...
    db.run(`UPDATE donation_goals SET 
        ...
        function(err) {
            ...
            console.log('✅ Цель обновлена, добавляем запись о донате');
            ...
            db.run(`INSERT INTO goal_donations (goal_id, amount, username, message, is_manual) 
                ...
                if (err) {
                    ...
                    console.log('✅ Запись о донате добавлена');
                }
...
// Получить состояние полоски сбора
...
// Обновить состояние полоски сбора
...
// Добавить сумму к полоске (при донате)
...
// --- Виджет «параметр от донатов» (процент/единицы, за N₽ +X, лимит) ---
